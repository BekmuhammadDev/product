// const svaeBtn = document.querySelector("#save-btn")
...
function searchProduct() {
   ...
   if (option === "name") {
      ...
      name.toLowerCase();
      result = shop.filter((item) => item[option].toLowerCase().includes(name));
   } else if (option === "id") {
      id = +prompt(`${option} ni kiritin`);
      result = shop.filter((item) => item[option].includes(id));
   }
...
function searchProduct() {
   ...
   while (!["name", "id"].includes(option)) {
       alert("Siz faqat name va id ni kiritishingiz mumkin!");
       option = prompt("Siz nima boyicha search qilmoqchisiz (ID/name): ").toLowerCase();
   }
